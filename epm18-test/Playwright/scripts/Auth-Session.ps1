...
if ([string]::IsNullOrWhiteSpace($UserKey) -or $UserKey -eq "user") {
  $UserKey = "user"
}

if ([string]::IsNullOrWhiteSpace($PodKey)) {
  if ($podSettings -and $podSettings.podKey) {
    $PodKey = [string]$podSettings.podKey
  }
  else {
    $PodKey = Read-Host "Enter pod key (optional, e.g. 11_test_fccs). Leave blank for default"
  }
}
$PodKey = Normalize-Key -Value $PodKey

$fileStem = if ([string]::IsNullOrWhiteSpace($PodKey)) { $UserKey } else { "$UserKey.$PodKey" }
$storageStatePath = "playwright/.auth/$fileStem.json"

if ([string]::IsNullOrWhiteSpace($BaseUrl)) {
  if ($podSettings -and $podSettings.podUrl) {
    $BaseUrl = [string]$podSettings.podUrl
  }
}
...
if ([string]::IsNullOrWhiteSpace($BaseUrl)) {
  $BaseUrl = Read-Host "Enter pod URL (https://...)"
}
...
New-Item -ItemType Directory -Path "playwright/.auth" -Force | Out-Null

$envFile = if ($env:ENV_FILE) { $env:ENV_FILE } elseif (-not [string]::IsNullOrWhiteSpace($PodKey)) { ".env.$PodKey" } else { ".env" }
Set-Or-AppendEnvValue -Path $envFile -Name "PW_STORAGE_STATE" -Value $storageStatePath
...
if (Test-Path -LiteralPath $storageStatePath) {
  $args += "--load-storage=$storageStatePath"
}

if ($Mode -eq "refresh") {
  $args += "--save-storage=$storageStatePath"
}
